...
const Login = ({setLoginUser}) => {
  ...
  const register=  (e)=>{
    ...
    if(name&&email&&password){
       axios.post("http://localhost:8000/register",userSignOut).then(res=>alert(res.data.message))
    }
    else{
      alert("invalid input")
    }
      ...
      <div className={styles["form-container"] + " " + styles["sign-up-container"]}>
        <form method="POST">
          ...
          <input
            ...
            onChange={handlechange}
          />
          <button style={{ backgroundColor: buttonColor }} onClick={register}>Sign Up</button>
        </form>
      </div>
